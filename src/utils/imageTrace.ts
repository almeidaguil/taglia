...
function parsePathD(d: string): Array<Array<[number, number]>> {
  const tokens = d.trim().match(/[MmLlHhVvCcSsQqTtZz]|[+-]?[0-9]*\.?[0-9]+(?:e[+-]?[0-9]+)?/gi) ?? []
  ...
  while (i < tokens.length) {
    ...
    if (C === 'M') {
      if (cur.length > 0) { subPaths.push(cur); cur = [] }
      ...
      while (hasNum()) {
        const p1: [number, number] = prevCtrl ? [2*cx - prevCtrl[0], 2*cy - prevCtrl[1]] : [cx, cy]
        const p2 = rel(abs, rn(), rn())
        const p3 = rel(abs, rn(), rn())
        cur.push(...sampleCubic([cx, cy], p1, p2, p3))
        prevCtrl = p2;
 ...
 */
function nestingLevel(idx: number, paths: Array<Array<[number, number]>>): number {
 ...
 */
function removeBackground(gray: Uint8Array, w: number, h: number, tolerance = 28): void {
...
function parseSvgToPaths(svgString: string): SvgParseResult {
  ...
  if (tm) {
    ...
    if (sc) { scaleX = parseFloat(sc[1]); scaleY = parseFloat(sc[2] ?? sc[1]) }
  }
  ...
  const outerEntry = filtered.reduce((best, cur) => cur.area > best.area ? cur : best)
...
export async function traceImageToScadPolygon(
  ...
  const MAX_PX = 1024
  const scaleFactor = Math.min(1, MAX_PX / Math.max(img.naturalWidth, img.naturalHeight))
  ...
  // 6. Potrace → SVG
  const svgString = await potrace(padded, { turdsize: 4, optcurve: true, opttolerance: 0.3 })
  ...
  if (outer.points.length === 0) {
    throw new Error('Nenhum contorno encontrado na imagem. Tente aumentar o contraste.')
  }
  ...
  const toScad = (pts: Array<[number, number]>) => {
    const pStr = '[' + pts.map(([x, y]) => `[${x.toFixed(3)},${y.toFixed(3)}]`).join(',') + ']'
  ...
  const subpathsScad    = all.paths.map(idxList => toScad(idxList.map(i => scaledAll[i])))
  const subpathsByLevel = byLevel.map(lvlPaths => lvlPaths.map(sub => toScad(tf(sub))))
